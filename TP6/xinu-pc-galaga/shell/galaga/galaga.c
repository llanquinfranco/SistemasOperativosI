...
#define PIERDE_VIDA 1
#define GANA_PUNTO 2
#define RESETEAR 3

pid32 pidJuego;
pid32 pidVidasPuntaje;
pid32 pidControl;
pid32 pidDriverTeclado;
...
int galaga(void) {
	...
	while(1) {
		...
		// Detecta que el jugador quiere terminar
		if(KEY_DOWN_NOW(BUTTON_ESC)) {
			// Envia al proceso 3 un mensaje de que el juego termino y que el jugador quiere salir del juego
			send(pidControl, -1);
		}
		...
		for (int a = 0; a < 9; a++) {
			...
			if (collision(easyEnemies[a].enemyX, easyEnemies[a].enemyY, 20, 20, player.playerX, player.playerY)) {
				// Detecta que la nave player es alcanzada por una nave enemiga
				drawRect(easyEnemies[a].enemyX, easyEnemies[a].enemyY,  20, 20, BLACK);
				easyEnemies[a].enemyY = 0;	// Elimino la nave enemiga
				endGame();
			}	
			...
			if (shoots[i] != 0) {
				...
				for (int j = 0; j < 9; j++) {
					if (collision(easyEnemies[j].enemyX, easyEnemies[j].enemyY, 15, 15, shoots[i] % 240, shoots[i] / 240)) {
						// Detecta que un disparo colisionó con una nave enemiga
						if(!colisionDetectada) {
							// Envia un mensaje al proceso 2 para que aumente el puntaje de player
							send(pidVidasPuntaje, GANA_PUNTO);
							colisionDetectada = 1;
						}
						// Elimino la nave enemiga
...
void endGame() {
	// Envia un mensaje al proceso 2 para indicar que el player perdio una vida
	send(pidVidasPuntaje, PIERDE_VIDA);
...
void vidasYPuntaje() {
	// Proceso 2 (Mantiene las vidas y el puntaje)
	char buffer[64];
	int32 mensaje;
	vidas = 3;
	puntaje = 0;
	...
	while(1) {
		sprintf(buffer, "- Vidas restantes: %d - Puntaje: %d -", vidas, puntaje);
...
void teclado() {
	// Nuevo proceso que utiliza el nuevo driver del teclado
	char c;
...
void controlGalaga() {
	// Proceso 3 (Control)
	pidControl = currpid;
	pidVidasPuntaje = create(vidasYPuntaje, 1024, 20, "Vidas y Puntaje", 0);
	pidDriverTeclado = create(teclado, 1024, 20, "Proceso Teclado", 0);
	pidJuego = create(galaga, 1024, 20, "Juego Galaga", 0);

	resume(pidVidasPuntaje);
	resume(pidDriverTeclado);
	resume(pidJuego);

	receive();
	// Cuando recibe el mensaje, finaliza el juego
	kill(pidVidasPuntaje);
	kill(pidDriverTeclado);
	kill(pidJuego);
	kill(pidControl);
}